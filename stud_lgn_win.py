...
class Login():
    def __init__(self,root):
        ...
        Label(Frame_login, text="STUDENT NAME", font=("Goudy old style", 15, "bold"), background="black", fg="white").place(x=150,y=60)
        ...
        Label(Frame_login, text="SECTION", font=("Goudy old style", 15, "bold"), background="black", fg="white").place(x=150, y=270)
        self.txt_sec = ttk.Combobox(Frame_login, font=("times new roman", 10),state="readonly")
        self.txt_sec["values"] = ("SELECT", "A")
        self.txt_sec.place(x=250, y=272, width=70, height=25)
        self.txt_sec.current(0)

        Label(Frame_login, text="CLASS", font=("Goudy old style", 15, "bold"), background="black", fg="white").place(x=368, y=270)
        ...
        self.txt_class.place(x=440, y=272, width=70, height=25)
        self.txt_class.current(0)

        Label(Frame_login, text="ROLL NO", font=("Goudy old style", 15, "bold"), background="black", fg="white").place(x=150, y=310)
        self.txt_roll = Entry(Frame_login, font=("times new roman", 15), bg="white")
        self.txt_roll.place(x=250, y=310, width=70, height=25)
    ...
    def checkdata(self):
        if self.txt_roll.get()=="" or self.txt_sec.get()=="SELECT" or self.txt_class.get()=="SELECT" or self.txt_passwd.get()=="" or self.txt_user.get()=="" :
            ...
            try:
                ...
                cur=con.cursor()
                cur.execute(f"select * from class{self.txt_class.get()}{self.txt_sec.get()} where roll_no=%s and student_name=%s and username=%s and password=%s",(
                    self.txt_roll.get(),
                    self.txt_user.get(),
